      ...
      <div className="cart">
        ...
        {props.shoppingCart.map((product, indx) => (
          <div className="product-display" key={product.itemId}>
            <p className="cart-product-name">{findName(product.itemId)}</p>
            <p className="receipt-quantity">: {product.quantity}</p>
          </div>
        ))}
      </div>
      <p className="subTotal: ">Subtotal: ${props.price.toFixed(2)}</p>
      <p className="total">
